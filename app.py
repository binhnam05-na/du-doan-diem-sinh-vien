...
df_students["TongDiem"] = df_students[mon_selected].sum(axis=1)
...
# -------------------------
# Thêm phân cụm K-Means
# -------------------------
k = st.sidebar.slider("Số cụm K-Means", 2, 6, 3)
X_cluster = df_students[["TongDiem"] + nganh_list].copy()

# Xử lý NaN/inf trước khi chạy K-Means
...
with col2:
    ...
    st.subheader("📊 Phân cụm sinh viên theo điểm & xác suất trúng tuyển")
    fig3, ax3 = plt.subplots(figsize=(10,4))
    scatter = ax3.scatter(df_students["TongDiem"], df_students[nganh_list[0]],
                          c=df_students["Cluster"], cmap="viridis", alpha=0.6)
    ax3.set_xlabel(f"Tổng điểm 3 môn ({khoi_selected})")
    ax3.set_ylabel(f"Xác suất {nganh_list[0]}")  # chỉ hiển thị 1 ngành để trực quan
    ax3.set_title("Phân cụm sinh viên theo K-Means")
